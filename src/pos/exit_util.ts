...
export class ExitUtil {
  ...
  private getLogIndex_(logEventSig: string, receipt: ITransactionReceipt, token = '') {
    let logIndex = -1;
    ...
    switch (logEventSig) {
      ...
      case '0xf94915c6d1fd521cee85359239227480c7e8776d7caf1fc3bacad5c269b66a14':
        for (let i = 0; i < receipt.logs.length; i++) {
          ...
          if (token !== '' && token.toLowerCase() !== log.address.toLowerCase()) continue;
  ...
  buildPayloadForExit(burnTxHash: string, logEventSig: string, isFast: boolean, token = '') {
