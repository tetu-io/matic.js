import {ITransactionWriteResult} from "../interfaces";
...
export class TransactionWriteResult implements ITransactionWriteResult {
  ...
  constructor(private promise: any) {
    ...
    promise.once("transactionHash", this.onTransactionHash).once("receipt", this.onTransactionReceipt as any).on("error", this.onTransactionError).on("error", this.onTransactionReceiptError);
  }
